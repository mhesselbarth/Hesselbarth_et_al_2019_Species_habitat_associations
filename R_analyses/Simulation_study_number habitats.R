...
Simulation.Habitat.Randomization <- function(number_coloumns, number_rows, 
                                             ...
                                             simulation_runs, number_habitats, alpha_sequence){
  # Data frames for results
  detections_tibble <- tibble::tibble(Species=numeric(), Habitats=numeric(), Alpha=numeric(), Correct=numeric(), False=numeric())
  
  # Loop over association strengths
  for(i in 1:length(number_habitats)){
    ...
    simulation_habitats <- NLMR::nlm_mpd(ncol=number_coloumns, nrow=number_rows, 
                                         ...
                                         verbose=F) %>%
      SHAR::Habitat.Classification(classes=number_habitats[i])
    ...
    for(j in 1:simulation_runs){
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=1, Habitats=number_habitats[i], Alpha=alpha,
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=2, Habitats=number_habitats[i], Alpha=alpha,
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=3, Habitats=number_habitats[i], Alpha=alpha,
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=4, Habitats=number_habitats[i], Alpha=alpha,
    ...
    print(paste0(i , " from ", length(number_habitats), " possible number of habitats done"))
    cat("\n\n")
  }
...
Simulation.Point.Process <- function(number_coloumns, number_rows,
  ...
  for(i in 1:length(number_habitats)){
    ...
    print(paste0(i , " from ", length(number_habitats), " possible alphas done"))
    cat("\n\n")
  }
...
#                   file=paste0(results,"/number_habitats_torus_translation.rds"))
...
#                   file=paste0(results,"/number_habitats_habitat_randomization.rds"))
...
#                   file=paste0(results,"/number_habitats_point_process.rds"))
...
#                   file=paste0(results,"/number_habitats_pattern_reconstruction.rds"))
