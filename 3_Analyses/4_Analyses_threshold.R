...
Simulation.Habitat.Randomization <- function(number_coloumns, number_rows, 
                                             ...
                                             simulation_runs, threshold_list, alpha_sequence){
  # Data frames for results
  detections_tibble <- tibble::tibble(Species=numeric(), Threshold=numeric(), Alpha=numeric(), 
  ...
  for(i in 1:length(threshold_list)){
    ...
    for(j in 1:simulation_runs){
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=1, Threshold=threshold_list[[i]][1]*2, Alpha=alpha,
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=2, Threshold=threshold_list[[i]][1]*2, Alpha=alpha,
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=3, Threshold=threshold_list[[i]][1]*2, Alpha=alpha,
      ...
      detections_tibble[nrow(detections_tibble)+1,] <- c(Species=4, Threshold=threshold_list[[i]][1]*2, Alpha=alpha,
    ...
    print(paste0(i , " from ", length(threshold_list), " possible number of habitats done"))
    cat("\n\n")
  }
...
Simulation.Point.Process <- function(number_coloumns, number_rows,
  ...
  for(i in 1:length(threshold_list)){
    ...
    print(paste0(i , " from ", length(threshold_list), " possible alphas done"))
    cat("\n\n")
  }
...
threshold_list <- list(c(0.005, 0.995), c(0.025, 0.975), c(0.05, 0.95))
...
#                   file=paste0(results,"/thresholds_torus_translation.rds"))
...
#                   file=paste0(results,"/thresholds_habitat_randomization.rds"))
...
#                   file=paste0(results,"/thresholds_point_process.rds"))
