...
#' @export
create_simulation_species <- function(raster, type, process, habitat, number_points=100, alpha=0.3,
  ...
  if(type=="positive"){
    ...
    if(process=="Poisson"){
      ...
      if(verbose==T){print("Please select either 'Poisson', 'Thomas' or as process")}
      pattern <- NULL
    }
  }
